...
function listData(time, periods) {
	...
	$.ajax({
		url: urlbublic + "CQShiCai/queryCQShiCaiTrendByIssue.do?lotCode=" + lotCode + "&issue=" + periods + "&date=" + time,
		type: "GET",
		data: {
			'lottObj': ""
		},
...
function creatDataHead(jsondata) {
	...
	//放进localstory
	localStorage.drawCode = data.drawCode;
	localStorage.nextIssue = data.nextIssue;
	if(config.ifdebug) {
		console.log("nextIssue:" + localStorage.nextIssue);
	}
	var drawCode = (data.drawCode).split(",");
...
function forRank(data) {
	...
	$("#trend_table2 tbody").append("<tr>" + tr1 + "</tr><tr>" + tr2 + "</tr><tr>" + tr3 + "</tr><tr>" + tr4 + "</tr>");
}
//执行画线
function drawLine() {
