...
animateMethod.sscAnimate = function(id) {
		...
		intervalSsc = setInterval(function() {
			...
			for(var i = 0; i < lilength; i++) {
				...
				//为li产生随机数字
				$(id).find("li").eq(i).text(excutenum());
				//追加动画
			...
			if(time == 100) {
				...
				if(config.ifdebug) {
					console.log("球动画时id=" + intervalSsc[id] + ";球动画time:" + JSON.stringify(animateID));
				}
				clearInterval(animateID[id]);
				ajaxRequst($(id).find(".nextIssue").text(), "", $(id).attr("id")); //请求后台加载数据传入一下期期数
			}
		...
		if(config.ifdebug) {
			console.log("动画ID" + JSON.stringify(animateID[id]));
		}
	}
	//时时彩终极动画
