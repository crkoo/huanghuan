...
if (!empty($_FILES)) {
	...
	$newFile = time().'.'.$fileParts['extension'];
	
	if (in_array(strtolower($fileParts['extension']),$fileTypes)) {
		if (!move_uploaded_file($tempFile,$targetFile.$newFile)){
            echo 'Invalid file type.';
            exit;
        }
		$array = array('fileUrl'=>$host.'/uploads/'.$newFile,'fileName'=>$fileParts['basename']);
        if ($width || $height){
            execute($_SERVER['DOCUMENT_ROOT'].'/uploads/'.$newFile, $width, $height, '/uploads/');
        }
