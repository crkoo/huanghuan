...
$(window).load(function() {
    ...
    if(pageSite && (pageSite.toLowerCase() == 'game' || pageSite.toLowerCase() =='game3dfight')) {
        var unityObjectUrl = "skins/js/game/UnityObject.js";
...
f_com.bm = function(url, n, o) {
    ...
    //    	unityObject.embedUnity("unityPlayer", "skins/swf/TestUnity.unity3d", '0', '0', null, null, function(result){
    //            if (!result.success) {
...
f_com.getPager = function(type, mo, me) {
    ...
    if(type.charAt(0) == '#') {
        ...
        if(type == '_bank') {
         ...
         top.mem_index.location.href = 'skins/?module=System&method=' + mo + '&other=' + me;
    } else {
        if($(HID).length == 1) {
            ...
            top.mem_index.location.href = 'skins/?module=System&method=' + type + '&other=' + mo;
        }
    }
...
f_com.getGamePager = function(type, me, id, gameIndex) {
    ...
    if(type.charAt(0) == '-') {
        top.mem_index.location.href = 'skins/?module=System&method=' + me + '&gameIndex=' + gameIndex;
    } else {
        $("div[id^='" + id + "-']").hide();
...
function HotNewsHistory() {
    // var features = 'height=600,width=800,top=0, left=0,scrollbars=yes,resizable=yes';
     //window.open('skins/?module=MFunction&method=ShowHotNewsHistory', 'HotNewsHistory', features);
}
...
function GetGameQuantity(color){
   $.getJSON('skins/?module=MCountGameNums&method=CountGames',{ "noCache": Math.random()}, function(data){
...
function GetFavorQuantity(color) {
    $.getJSON('skins/?module=MCountGameNums&method=CountFavoGames', {"noCache": Math.random()}, function(data) {
...
function gameRule(GAME_TYPE) {
    window.open('skins/?module=MRule&method=ruleDescrip&args=' + GAME_TYPE, 'AllRule', 'width=1024,height=640,status=no,scrollbars=yes');
}
...
f_com.MGetPager = function(mo, me) {
    window.open("skins/?module=" + mo + "&other=" + me, "MACENTER", "top=50,left=50,width=1020,height=610,status=no,scrollbars=yes,resizable=no").focus();
};
...
f_com.BBinGetPager = function(mo) {
    window.open("skins/?module=" + mo, "bb", "top=50,left=50,width=1000,height=600,status=no,scrollbars=no,resizable=yes").focus();
};
...
f_com.BBinEnterPager = function(mo,me,ty) {
    //視窗寬度ub連覽器判斷
    if (!ty){
        window.open("skins/?module=" + mo +"&PageItem=" + me , "BBinPage", "top=50,left=50,width=1000,height=600,status=no,scrollbars=no,resizable=yes").focus();
    }else{
        window.open("skins/?module=" + mo +"&PageItem=" + me+"&type=" + ty , "BBinPage", "top=50,left=50,width=1000,height=600,status=no,scrollbars=no,resizable=yes").focus();
    }
};
/*開啟bbin資訊中心-指定頁面並無左側選單*/
f_com.BBinOnlyPager = function(mo,me,ty)  {
    if (!ty){
        window.open("skins/?module=" + mo +"&OnlyPage=Y&PageItem=" + me , "BBinPage", "top=50,left=50,width=770,height=700,status=no,scrollbars=no,resizable=yes").focus();
        return;
    }

    window.open("skins/?module=" + mo +"&PageItem=" + me+"&OnlyPage=Y&type=" + ty , "BBinPage", "top=50,left=50,width=770,height=700,status=no,scrollbars=no,resizable=yes").focus();
}
/*event活動開啟*/
