 ...
 */
if ($a == 'index'){
    $list = $db->getLineAll("select * from dbl_admin");
}elseif ($a == 'edit'){
    if (!empty($_POST)){
        ...
        if (empty($id)){
            ...
            $db->insert('dbl_admin', array('admin_name' => $username, 'admin_password' => md5($pwd), 'admin_number' => 0, 'admin_status' => 1));
            ShowMsg("添加成功", 'index.php?m=ucenter&a=index');
        }else{
            $find = $db->getLine("select * from dbl_admin where id=".$id);
            if (empty($find)){
                ShowMsg('用户数据不存在', -1);
                exit;
            }
            ...
            $db->update('dbl_admin', $find['id'], "`admin_password`='".md5($pwd)."'");
    ...
    if (!empty($id)){
        $data = $db->getLine("select * from dbl_admin where id=".$id);
    }else{
        $data = array(
            'id' => null
        );
    ...
    if ($_POST){
        ...
        $find = $db->getLine("SELECT admin_password FROM `dbl_admin` WHERE id={$mid} ");
        if (!$find){
            unset($_SESSION['uid']);
            unset($_SESSION['nick']);
        ...
        $result = $db->query("UPDATE `dbl_admin` SET `admin_password`='{$newpwd}' WHERE id={$mid}");
        if ($result){
            ShowMsg('修改密码成功!',-1);
            exit();
        }else{
    ...
    $user = $db->getLine("select id from dbl_admin where id=$id");
    if(empty($user)){
        outputJson(1, '数据不存在');
    }
    $db->update('dbl_admin', $id, "`admin_status`=".$status);
    outputJson(0, 'ok');
}else{
